...
public class CryptoTest {
    ...
    public void backups() throws Exception {
        final byte[] backup = Crypto.decryptBytes(readBackupFromResource("ctp-wallet-backup-testnet-5.18"),
                PASSWORD);
        assertTrue(WalletProtobufSerializer.isWallet(new ByteArrayInputStream(backup)));

        final byte[] backupCrLf = Crypto.decryptBytes(readBackupFromResource("ctp-wallet-backup-testnet-5.18-crlf"),
                PASSWORD);

...
public final class RequestWalletBalanceTask {
	...
	boolean requestWalletBalanceFromBlockExplorers(Address address) {
		Set<UTXO> utxos = requestWalletBalanceFromBlockExplorer("https://insight.ctp.org/api/addr/", UnspentAPI.Insight, address);

		if(utxos != null) {
			onResult(utxos);
			return true;
		} else {
			utxos = requestWalletBalanceFromBlockExplorer("https://explorer.ctp.org/chain/Ctp/unspent/", UnspentAPI.ABE, address);
			if(utxos != null) {
				onResult(utxos);
				return true;
			} else {
				utxos = requestWalletBalanceFromBlockExplorer("https://chainz.cryptoid.info/ctp/api.dws?q=unspent", UnspentAPI.CryptoId, address);
				if(utxos != null) {
					onResult(utxos);
					return true;
				}

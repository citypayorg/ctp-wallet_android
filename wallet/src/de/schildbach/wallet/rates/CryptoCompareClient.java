...
public class CryptoCompareClient extends RetrofitClient {
    ...
    private CryptoCompareClient(String baseUrl) {
        ...
        Moshi moshi = moshiBuilder.add(new BigDecimalAdapter())
                .add(new CryptoCompareCtpBtcRateAdapter()).build();
    ...
    public Response<Rate> getCtpCustomAverage() throws IOException {
        return service.getCtpCustomAverage().execute();
    }
    ...
    private interface CryptoCompareService {
        @GET("data/generateAvg?fsym=CTP&tsym=BTC&e=Binance,Kraken,Poloniex,Bitfinex")
        Call<Rate> getCtpCustomAverage();

...
public final class BitcoinIntegration {
    ...
    private static final String MIMETYPE_PAYMENTREQUEST = "application/ctp-paymentrequest"; // BIP 71
    ...
    private static Intent makeBitcoinUriIntent(final String address, final Long amount) {
        final StringBuilder uri = new StringBuilder("ctp:");
        if (address != null)
            uri.append(address);
        if (amount != null)
            uri.append("?amount=")
    ...
    private static void redirectToDownload(final Context context) {
        Toast.makeText(context, "No Ctp application found.\nPlease install Ctp Wallet.", Toast.LENGTH_LONG)
                .show();

        final Intent marketIntent = new Intent(Intent.ACTION_VIEW,
                Uri.parse("market://details?id=citypay.darkcoin.wallet"));
        final Intent binaryIntent = new Intent(Intent.ACTION_VIEW,
                Uri.parse("https://github.com/CityPayOrg/ctp-wallet/releases"));

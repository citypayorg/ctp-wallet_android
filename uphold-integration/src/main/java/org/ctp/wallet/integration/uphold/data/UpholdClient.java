...
package org.ctp.wallet.integration.uphold.data;
...
public class UpholdClient {
    ...
    private void getCards(final Callback<String> callback, final Callback<UpholdCard> getCtpCardCb) {
        service.getCards().enqueue(new retrofit2.Callback<List<UpholdCard>>() {
            ...
            public void onResponse(Call<List<UpholdCard>> call, Response<List<UpholdCard>> response) {
                if (response.isSuccessful()) {
                    ...
                    ctpCard = getCtpCard(response.body());
                    if (ctpCard == null) {
                        log.info("Ctp Card not available");
                        createCtpCard(callback, getCtpCardCb);
                    } else {
                        if (ctpCard.getAddress().getCryptoAddress() == null) {
                            log.info("Ctp Card has no addresses");
                            createCtpAddress(ctpCard.getId());
                        }
                        callback.onSuccess(ctpCard.getId());
                        if (getCtpCardCb != null) {
                            getCtpCardCb.onSuccess(ctpCard);
                        }
                    }
                } else {
    ...
    private void createCtpCard(final Callback<String> callback, final Callback<UpholdCard> getCtpCardCb) {
        Map<String, String> body = new HashMap<>();
        body.put("label", "Ctp Card");
        body.put("currency", "CTP");
        service.createCard(body).enqueue(new retrofit2.Callback<UpholdCard>() {
            ...
            public void onResponse(Call<UpholdCard> call, Response<UpholdCard> response) {
                if (response.isSuccessful()) {
                    log.info("Ctp Card created successfully");
                    ctpCard = response.body();
                    String ctpCardId = ctpCard.getId();
                    callback.onSuccess(ctpCardId);
                    createCtpAddress(ctpCardId);
                    if (getCtpCardCb != null) {
                        getCtpCardCb.onSuccess(response.body());
                    }
                } else {
                    log.error("Error creating Ctp Card: " + response.message() + " code: " + response.code());
                    callback.onError(new UpholdException("Error creating Ctp Card", response.message(), response.code()), false);
                }
            }

            @Override
            public void onFailure(Call<UpholdCard> call, Throwable t) {
                log.error("Error creating Ctp Card " + t.getMessage());
            }
        });
    }

    private void createCtpAddress(String cardId) {
        Map<String, String> body = new HashMap<>();
        body.put("network", "ctp");
        service.createCardAddress(cardId, body).enqueue(new retrofit2.Callback<UpholdCryptoCardAddress>() {
            ...
            public void onResponse(Call<UpholdCryptoCardAddress> call, Response<UpholdCryptoCardAddress> response) {
                log.info("Ctp Card address created");
            }

            @Override
            public void onFailure(Call<UpholdCryptoCardAddress> call, Throwable t) {
                log.error("Error creating Ctp Card address: " + t.getMessage());
            }
        });
    }

    private UpholdCard getCtpCard(List<UpholdCard> cards) {
        for (UpholdCard card : cards) {
            if(card.getCurrency().equalsIgnoreCase("ctp")) {
                return card;
            }
        }
        return null;
    ...
    public void getCtpBalance(final Callback<BigDecimal> callback) {
        getCards(new Callback<String>() {
            ...
            public void onError(Exception e, boolean otpRequired) {
                log.error("Error loading Ctp balance: " + e.getMessage());
    ...
    public void createCtpWithdrawalTransaction(String amount, String address,
        ...
        denomination.put("currency", "CTP");
        body.put("denomination", denomination);
        body.put("destination", address);

        service.createTransaction(ctpCard.getId(), body).enqueue(new retrofit2.Callback<UpholdTransaction>() {
            @Override
    ...
    public void commitTransaction(String txId, final Callback<Object> callback) {
        service.commitTransaction(ctpCard.getId(), txId).enqueue(new retrofit2.Callback<Object>() {
            @Override
    ...
    public UpholdCard getCurrentCtpCard() {
        return ctpCard;
    }
